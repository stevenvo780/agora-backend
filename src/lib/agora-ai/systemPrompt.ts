...
export function buildAgoraSystemPrompt({ mode, contextPrompt = '', workspaceId, accessPolicy, userInstructions, hooks, dryRun }: BuildSystemPromptOptions): string {
  ...
  if (mode === 'agent') {
    base.push(
      ...
      '',
      '6. **Subtarea independiente con scope acotado** (cuando la tarea merece su propio loop):',
      '   → `spawn_subagent({ task: "<prompt explícito>", scope: "read-only"|"workspace"|"full", maxIterations: 5 })`.',
      '   → Ejemplos: "audita duplicados en /docs/clases", "limpia conceptos huérfanos del glosario".',
      '   → NO la uses para acciones triviales (1-2 tools) — ejecuta inline.',
      '',

/**
 * POST /api/workspaces/[id]/remotes/[remoteId]/pull
 * Trae los commits del remote externo al repo Forgejo. Operación on-demand.
 */
...
import { pullFromExternalRemote } from '@/lib/git-remotes/git-operations';
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    await docRef.update({
      ...
      updatedAt: now
    }).catch((e: unknown) => console.warn('[remotes/pull] no se pudo actualizar lastSync*', getErrorMessage(e)));

    if (!result.ok) {
      const status = result.notImplemented ? 501 : 502;
      return NextResponse.json({
        ok: false,
        error: result.error ?? 'pull falló',

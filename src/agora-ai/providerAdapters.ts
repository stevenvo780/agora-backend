import { buildAgoraSystemPrompt, extractThinkingSegments } from './systemPrompt.js';
import { toAnthropicTools, toGeminiTools, toOpenAITools } from './toolDefinitions.js';
import type {
...
// El executor de tools se inyecta vía AgentExecutionContext.executeAgentTool
// (en AgoraBackend la llamada se delega al hub Next.js por HTTP).
async function executeAgentTool(
  call: AgentToolCall,
  ctx: AgentExecutionContext
): Promise<AgentToolExecutionResult> {
  if (typeof ctx.executeAgentTool !== 'function') {
    return {
      ok: false,
      name: call.name,
      callId: call.id,
      summary: 'AgentExecutionContext.executeAgentTool no está cableado en este servidor.',
      error: 'tool_executor_not_wired'
    };
  }
  return ctx.executeAgentTool(call, ctx);
}
